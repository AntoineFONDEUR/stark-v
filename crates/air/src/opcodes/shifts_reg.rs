//! Register shift opcodes (sll/srl/sra) as a felt function (airs.md Section 3).
//! One-hot bit/limb markers decode the shift amount; carries chain the shifted
//! bytes and sign fill is applied on arithmetic right shifts. The flag sum is
//! the row activity indicator `enabler()`.
...
stwo_macros::define_air_fns! {
    ...
    fn shifts_reg(
        ...
        let expected_opcode_id = opcode_sll_flag * constant(crate::instructions::Opcode::Sll as u32)
        ...
        let limb_marker_sum = limb_shift_marker_0 + limb_shift_marker_1 + limb_shift_marker_2
                    + limb_shift_marker_3;
        let shift_check = (pow2(3) - 1) - (rs2_next_0 - shift_amount) * inv(pow2(5));
        let pc_next = pc + 4;
        ...
        consume range_check_20(shift_check);
