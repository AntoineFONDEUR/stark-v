        ...
        >,
{
    ...
    let preprocessing = preprocess_trace_with_channel::<MC>(
        config,
...
/// Commits a caller-owned preprocessing registry with the standard cached format.
///
/// Recursion uses this entry point because its universal preprocessing is
/// generated by the same AIR roster that defines the outer proof geometry.
pub fn preprocess_trace_with_channel<MC: stwo::core::channel::MerkleChannel>(
    config: PcsConfig,
    ids: Vec<stwo_constraint_framework::preprocessed_columns::PreProcessedColumnId>,
    trace: Vec<
        ...
        >,
{
    ...
    let max_log_size = trace.iter().map(|c| c.domain.log_size()).max().unwrap_or(0);
    ...
    let log_sizes: Vec<u32> = trace.iter().map(|c| c.domain.log_size()).collect();
    ...
    let ids: Vec<String> = ids.iter().map(|id| id.id.clone()).collect();
