//! Canonical protocol manifest for recursion current protocol.
//!
...
>
    ...
    >
{
    /// Commits the complete tagged manifest under the current protocol identity domain.
...
/// A PCS manifest value that cannot define the supported current protocol verifier.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::FriQueryCountOutOfRange { value } => {
                write!(
                    formatter,
                    "FRI query count {value} exceeds the current protocol maximum {MAX_FRI_QUERIES}"
                )
            }
