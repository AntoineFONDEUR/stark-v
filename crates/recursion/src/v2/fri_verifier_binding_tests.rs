//! End-to-end relation closure for the V2 FRI verification route bridge.
//!
...
fn fri_route_bridge_closes_between_control_and_input_airs() {
    ...
    for (reference, witness) in circuit_lanes(&references)
        .iter()
        .zip(circuit_lanes(&witnesses).iter())
    {
    ...
    let public_sum = circuit_lanes(&references)
        .iter()
        .map(|lane| {
