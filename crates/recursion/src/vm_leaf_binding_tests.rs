...
fn leaf_binding_relation_sum(mut channel: Poseidon2M31Channel) -> SecureField {
    let shape = shape();
    let vm_relation_challenge_count =
        u32::try_from(prover::relations::Relations::DESCRIPTORS.len())
            .expect("the VM relation registry fits the transcript counter");
